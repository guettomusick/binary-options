...
describe('BinaryOptions', function() {
  ...
  describe('Buy & Sell', () => {
    ...
    it('Default price must be 0.0001 ether', async () => {
      expect(await binaryOptions.getPrice(0)).to.equal(ethers.utils.parseEther(DEFAULT_PRICE.toFixed(18)));
    });
    ...
    it('should get tokens when buying at default price', async () => {
      ...
      const lockBalance = await binaryOptions.lockBalance(address.sender);
    ...
    it('should revert if wallet has not enough balance', async () => {
      ...
      expect(await binToken.balanceOf(address.sender)).to.equal(getTokens(1));
      // Lock confirmation
      expect((await binaryOptions.lockBalance(address.sender)).available).to.equal(getTokens(1));
    ...
    it('should get ether when selling tokens at default price from lock', async () => {
      ...
      // Buy confirmation
      expect((await binaryOptions.lockBalance(address.sender)).available).to.equal(getTokens(10));

      const prevBalance = await sender.getBalance();
      await binaryOptions.connect(sender).sell(getTokens(1), true, { gasPrice: 0 });
      // token transfer from lock
      expect((await binaryOptions.lockBalance(address.sender)).available).to.equal(getTokens(9));
      expect((await binaryOptions.lockBalance(address.sender)).total).to.equal(getTokens(9));
      // tokens burned
      expect(await binToken.totalSupply()).to.equal(getTokens(9));
      // ether transferred from contract
      expect(await ethers.provider.getBalance(address.binaryOptions)).to.equal(9);
  ...
  describe('Place, ExecuteRound and Unlock', () => {
    ...
    describe('Place', () => {
      it('should revert if amount is 0', async () => {
        await expect(
          binaryOptions.connect(sender).place(getNextPlayableRound(), 0, true, true)
        ).to.be.revertedWith('You need to send some BIN');
      });

      it('should revert if interval is not multiple of interval', async () => {
        await expect(
          binaryOptions.connect(sender).place(getNextPlayableRound()+1, getTokens(1), true, true)
        ).to.be.revertedWith('Timestamp must be multiple of interval');
      });

      it('should revert if betting for next or past round', async () => {
        await expect(
          binaryOptions.connect(sender).place(getNextRound(), getTokens(1), true, true)
        ).to.be.revertedWith('You can\'t bet for next round');

        await expect(
          binaryOptions.connect(sender).place(getLastRound(), getTokens(1), true, true)
        ).to.be.revertedWith('You can\'t bet for next round');

        await expect(
          binaryOptions.connect(sender).place(getOldRound(), getTokens(1), true, true)
        ).to.be.revertedWith('You can\'t bet for next round');
      });

      it('should place one option', async () => {
        const bet = 1;
        await binaryOptions.connect(sender).place(timeStamp, getTokens(bet), true, true);
        ...
        const pendingOption = await binaryOptions.pendingOptions(address.sender, 0);
      ...
      it('should add to both higher and lower at round', async () => {
        await binaryOptions.connect(sender).place(timeStamp, getTokens(.25), true, true);
        await binaryOptions.connect(sender).place(timeStamp, getTokens(.75), false, true);
        ...
        expect(await binaryOptions.getCollectedOptionsLength(address.sender)).to.equal(0);
    ...
    describe('Place & Execute', () => {
      it('should mark as executed, set price and revert if already executed', async () => {
        await binaryOptions.connect(sender).place(timeStamp, getTokens(1), true, true);
    ...
    describe('Place, Execute & Unlock', () => {
      it('should unlock if winner bet for higher', async () => {
        ...
        expect(await binaryOptions.getCollectedOptionsLength(address.sender)).to.equal(1);
        ...
        expect((await binaryOptions.getPrice(0)).toNumber()).to.approximately(ethers.utils.parseEther(
      ...
      it('should collect if winner bet for lower', async () => {
        const bet = 2;
        await binaryOptions.connect(sender).place(timeStamp, getTokens(bet), false, true);
        timeTravel(INTERVAL*2);
        await EthPriceFeed.mock.latestRoundData.returns(0, ETH_PRICE - 100, getLastRound(), getLastRound(), 0);
        await binaryOptions.executeRound(timeStamp);
